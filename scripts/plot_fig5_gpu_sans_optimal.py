# scripts/plot_fig5_gpu_sans_optimal.py
# ---------------------------------------------------------------------
# P1: Balanced-SINR maximization (GPU-friendly) WITHOUT Optimal curve
# Methods: ZF (equal power), RZF (equal power), optional Exhaustive (CPU)
...
def run_case(cfg: SystemConfig,
    ...
    ys_a_zf, ys_a_rzf, ys_a_ex = [], [], []
    ys_b_zf, ys_b_rzf, ys_b_ex = [], [], []
    ...
    for snr_db in snr_grid_db:
        ...
        acc_a = {"zf": 0.0, "rzf": 0.0, "ex": 0.0}
        cnt_a = {"zf": 0,   "rzf": 0,   "ex": 0}

        acc_b = {"zf": 0.0, "rzf": 0.0, "ex": 0.0}
        cnt_b = {"zf": 0,   "rzf": 0,   "ex": 0}
        ...
        for s in range(samples):
            ...
            # ---------- Small-scale only ----------
            Hs = complex_normal_tf([K, M])  # TF tensor
            HkM = Hs
    ...
    panel_a = (np.array(ys_a_zf), np.array(ys_a_rzf), np.array(ys_a_ex) if include_exhaustive else None)
    panel_b = None
    if include_large:
        panel_b = (np.array(ys_b_zf), np.array(ys_b_rzf), np.array(ys_b_ex) if include_exhaustive else None)
...
def save_curves_csv(csv_path, snr_grid_db, panel, include_exhaustive: bool):
    ...
    with open(csv_path, "w", newline="") as f:
        w = csv.writer(f)
        header = ["snr_dB", "zf_db", "rzf_db"]
        if include_exhaustive:
            header.append("ex_db")
        w.writerow(header)
        for i, snr_db in enumerate(snr_grid_db):
            row = [snr_db, float(y_zf[i]), float(y_rzf[i])]
            if include_exhaustive:

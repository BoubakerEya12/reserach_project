...
# ---------------------------------------------------------------------
# 3GPP Urban Macro (UMa) large-scale model from our channels_3gpp module.
try:
    from sim.channels_3gpp import gen_large_scale_3gpp_uma as UMAlarge
    from sim.channels_3gpp import log_distance_shadowing as UMAwrapper
    HAS_PATHLOSS = True
except Exception:
    # Safe fallback so the script doesn't crash if the module isn't present.
    import numpy as _np
    UMAlarge = None
    def UMAwrapper(*, rng, K, fc_GHz):
        # Return "no path-loss" gains (beta=1) and dummy distances
        return _np.ones(K, dtype=_np.float32), _np.zeros(K, dtype=_np.float32)
    HAS_PATHLOSS = False
...
def run_case(K, M, N, sigma2, samples, snr_grid_db, include_large, rng, verbose, U_max,
    ...
    for snr_db in snr_grid_db:
        ...
        for s in range(samples):
            ...
            if include_large:
                if (UMAlarge is not None) and HAS_PATHLOSS:

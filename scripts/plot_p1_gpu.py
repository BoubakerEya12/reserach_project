...
def build_subset_cache_tf(K: int, U_max: int):
    cache = {}
    for u in range(1, min(U_max, K) + 1):
        comb = np.array(list(itertools.combinations(range(K), u)), dtype=np.int32)
        cache[u] = tf.convert_to_tensor(comb, dtype=tf.int32)
    return cache


def best_group_balanced_sinr_exact_tf(HkM, sigma2, P_rb, subset_cache, mode="rzf"):
    """
    Exact max-min SINR over all subsets up to U_max, batched on GPU.
    Uses equal-power ZF/RZF beams.
    """
    best_val = tf.constant(0.0, dtype=tf.float32)
    reg = tf.constant(1e-6, dtype=tf.float32)

    for u, sel_all in subset_cache.items():
        c = tf.shape(sel_all)[0]
        H_sel = tf.gather(HkM, sel_all, axis=0)                         # [C,U,M]
        H = tf.transpose(H_sel, [0, 2, 1])                              # [C,M,U]
        Gram = tf.matmul(tf.linalg.adjoint(H), H)                       # [C,U,U]
        eye = tf.eye(u, batch_shape=[c], dtype=Gram.dtype)

        if mode == "zf":
            A = Gram + tf.cast(reg, Gram.dtype) * eye
        else:
            alpha = tf.cast(u, tf.float32) * tf.cast(sigma2, tf.float32) / tf.maximum(tf.cast(P_rb, tf.float32), 1e-12)
            A = Gram + tf.cast(alpha, Gram.dtype) * eye + tf.cast(reg, Gram.dtype) * eye

        V = tf.matmul(H, tf.linalg.inv(A))                              # [C,M,U]
        V = V / (tf.norm(V, axis=1, keepdims=True) + 1e-12)

        p_each = tf.cast(P_rb, tf.float32) / float(u)
        W = V * tf.cast(tf.sqrt(tf.maximum(p_each, 0.0)), V.dtype)      # [C,M,U]

        HW = tf.matmul(H_sel, W)                                         # [C,U,U]
        pwr = tf.abs(HW) ** 2
        sig = tf.linalg.diag_part(pwr)
        interf = tf.maximum(tf.reduce_sum(pwr, axis=-1) - sig, 0.0)
        sinr = sig / (interf + tf.cast(sigma2, sig.dtype))
        min_sinr = tf.reduce_min(sinr, axis=1)                           # [C]

        best_u = tf.reduce_max(tf.cast(min_sinr, tf.float32))
        best_val = tf.maximum(best_val, best_u)

    return max(float(best_val.numpy()), 1e-30)
...
def run_case(cfg: SystemConfig,
    ...
    U = min(cfg.U_max, K, M)
    subset_cache = build_subset_cache_tf(K, U)
    ...
    for snr_db in snr_grid_db:
        ...
        for s in range(samples):
            ...
            # Optimal (GPU exact over all subsets with equal-power RZF beams)
            opt_lin = best_group_balanced_sinr_exact_tf(HkM, sigma2, P_rb, subset_cache, mode="rzf")
            ...
            if include_large:
                ...
                opt_lin_b = best_group_balanced_sinr_exact_tf(HkM_eff, sigma2, P_rb, subset_cache, mode="rzf")
...
def plot_panel(snr_grid_db, panel, cfg: SystemConfig, samples: int, title: str, include_exhaustive: bool):
    ...
    ax.plot(snr_grid_db, y_opt, "o-", label="Optimal (GPU exact-subset)")

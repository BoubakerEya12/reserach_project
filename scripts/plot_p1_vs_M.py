...
from sim.config import SystemConfig   # unified configuration
...
def rzf_directions(H_sel, sigma2, P_rb):
    """RZF beam directions for selected users H_sel [U, M]."""
    U, M = H_sel.shape
    H = H_sel.T                       # [M,U]
    Gram = H.conj().T @ H             # [U,U]
...
def zf_equal_power_value(HkM, sigma2, P_rb, U_max):
    """Balanced SINR for ZF equal-power heuristic."""
...
def rzf_equal_power_value(HkM, sigma2, P_rb, U_max):
    """Balanced SINR for RZF equal-power heuristic."""
    sel = pick_subset_topU_by_norm(HkM, U_max)
    H_sel = HkM[sel, :]
...
def exhaustive_equal_power_value(HkM, sigma2, P_rb, U_max, mode="rzf"):
    """
    Exhaustive search over all user subsets (size 1..U_max), equal power.
    Returns the best min-SINR (linear).
    """
    K, M = HkM.shape
    users = list(range(K))
    best_min = 0.0
...
def mc_for_one_M(M, K, N, sigma2, P_rb, samples, rng,
    ...
    """
    Monte Carlo average of balanced SINR for a given (M,K).
    """
    if U_max is None:
        U_max = min(M, K)
    ...
    for s in range(samples):
        ...
        h_bs_ = apply_csit_model(h_eff, eta, rng) if eta < 1.0 else h_eff

        HkM = h_bs_[:, 0, :]  # [K,M], RB 0
...
def main():
    ...
    ap.add_argument("--K", type=int, default=None,
                    help="Number of users (fixed). If None, use cfg.K")
    ap.add_argument("--M-list", type=int, nargs="+", default=[2, 4, 6, 8],
                    help="List of BS antennas to evaluate, e.g. --M-list 2 4 6 8")
    ...
    ap.add_argument("--seed", type=int, default=None,
                    help="RNG seed (if None, use cfg.seed).")
    ap.add_argument("--eta", type=float, default=None,
                    help="CSIT quality in [0,1]. If None, use cfg.eta.")
    ...
    sigma2 = cfg.sigma2                      # N0 * B_RB from config
    P_rb   = sigma2 * db2lin(args.snr_db)    # per-RB transmit power
    ...
    ax.set_title(f"P1: Balanced SINR vs M  (K={K}, SNR={args.snr_db:.1f} dB, eta={eta})")
